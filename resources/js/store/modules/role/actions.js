const actions = {
	...
	updateRole (context, role) {
		context.commit('LOADING_STATUS', true, { root: true }) // start loading
		axios.put('api/roles/update', role)
			.then(response => {
				...
				let payload = [
					{ status: true, message: 'Role successfully updated.',
						timeout: 3000 },
				]
				context.commit('SNACKBAR_STATUS', payload, { root: true }) // show snackbar
			})
	...
	deleteRoles (context, roleIds) {
		console.log(roleIds)
		context.commit('LOADING_STATUS', true, { root: true }) // start loading
		axios.patch('api/roles/delete/multiple', roleIds)
			.then(response => {
				context.commit('DELETE_ROLES', response.data)
				context.commit('LOADING_STATUS', false, { root: true }) // stop loading

				let payload = [
					{ status: true, message: 'Role(s) successfully deleted.',
						timeout: 3000 },
				]
				context.commit('SNACKBAR_STATUS', payload, { root: true }) // show snackbar
			})
